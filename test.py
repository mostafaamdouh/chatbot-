...
- Closed: Every Friday
- Location: [Main Branch Fifth Settlement - Sec Branch Doki]
...
- مغلق: كل يوم جمعة
- الموقع: [الفرع الرئيسي التجمع الخامس - الفرع الثاني الدقي]
الخدمات:
...
def chat(user_message: str) -> str:
    ...
    # (Option A) Keep history but always prepend system message matching the current language
    ...
    # Save user+assistant messages (without system)
...
#chat { width:600px; margin:50px auto; background:white; padding:20px; border-radius:10px; }
#messages { height:300px; overflow:auto; border:1px solid #ddd; padding:10px; margin-bottom:10px; }
...
messages.innerHTML += "<p class='user'><b>You:</b> "+message+"</p>"
messages.innerHTML += "<p class='bot'><b>Bot:</b> "+data.reply+"</p>"
...
@app.route("/")
def home():
    return "AI Chatbot Server Running"
...
def api_chat():
    ...
    messages = [
        {"role":"system","content":AGENCY_DATA},
